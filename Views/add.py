from tkinter import Tk, Label, Entry, PhotoImage, Frame
...
class ade():
    ...
    def __init__(self):
        ...
        def focus_in5(event):
            if self.txt5.get() == "Telefono":
                self.txt5.delete(0, 'end')
                self.txt5.config(fg=backController.negro)

        def focus_out5(event):
            if self.txt5.get() == "":
                self.txt5.insert(0, 'Telefono')
                self.txt5.config(fg=backController.gris_claro)        
    
        self.window = Tk()
        self.window.geometry("1366x768+105+20") # Creación de ventana (1366x768)->Tamaño de la ventana (+105+20)posicion de la ventana
        self.window.title("Agregar Propietario") #titulo
        ...
        lbl = backController.create_label2('Agregar', self.window)#Titulo y estilos
        ...
        btn2 = backController.btn_inicio('Arrendatario', self.window, command=lambda:print("Arrendatario"))
        btn2.place(relx=0.05, rely=0.30, relheight=0.06, relwidth=0.18)
        
        btn2 = backController.btn_inicio('Agregar Propiedad', self.window, command=lambda:addController.house(self))
        ...
        lbl = Label(self.window, text = 'Propietario',font=("Arial", 28), bg=backController.blanco)
        ...
        self.txt1.insert(0, "Identificación") #Implementacion de placeholder
        ...
        self.txt2.insert(0, "Correo electronico") #Implementacion de placeholder
        ...
        self.txt3.insert(0, "Nombre") #Implementacion de placeholder
        self.txt3.bind("<FocusIn>", focus_in3) #Controlador de eventos
        self.txt3.bind("<FocusOut>",focus_out3) #Controlador de eventos
        
        #textbox con sus estilos
        self.txt4 = Entry(self.window,bg=backController.gris_oscuro, bd=0, highlightthickness=0, insertwidth=2, justify='center', font=("Arial", 18), fg=backController.gris_claro)
        self.txt4.place(relx=0.68, rely=0.40, relheight=0.05, relwidth=0.22)
        self.txt4.insert(0, "Apellido") #Implementacion de placeholder
        self.txt4.bind("<FocusIn>", focus_in4) #Controlador de eventos
        self.txt4.bind("<FocusOut>",focus_out4) #Controlador de eventos
        
        #textbox con sus estilos
        self.txt5 = Entry(self.window,bg=backController.gris_oscuro, bd=0, highlightthickness=0, insertwidth=2, justify='center', font=("Arial", 18), fg=backController.gris_claro)
        self.txt5.place(relx=0.55, rely=0.60, relheight=0.05, relwidth=0.22)
        self.txt5.insert(0, "Telefono") #Implementacion de placeholder
        self.txt5.bind("<FocusIn>", focus_in5) #Controlador de eventos
        self.txt5.bind("<FocusOut>",focus_out5) #Controlador de eventos
